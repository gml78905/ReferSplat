...
def overlay_mask_on_image(original_image, mask, alpha=0.5, color=[1.0, 0.0, 0.0]):
    """
    원본 이미지에 마스크를 오버레이합니다.
    
    Args:
        original_image: [3, H, W] 텐서 (원본 이미지)
        mask: [1, H, W], [3, H, W] 또는 [H, W] 텐서 (마스크, 0 또는 1)
        alpha: 마스크 투명도 (0~1)
        color: 마스크 색상 [R, G, B] (0~1 범위)
    ...
    if mask.dim() == 3:
        if mask.shape[0] == 3:
            # [3, H, W] -> 첫 번째 채널만 사용하거나 평균
            mask = mask[0]  # 또는 mask.mean(dim=0) 사용 가능
        else:
    ...
    # 마스크를 [3, H, W]로 확장하고 색상 적용
    mask_3d = mask.unsqueeze(0).repeat(3, 1, 1)  # [3, H, W]
    color_tensor = torch.tensor(color, device=device, dtype=original_image.dtype).view(3, 1, 1)  # [3, 1, 1]
    colored_mask = mask_3d * color_tensor  # [3, H, W]
    
    # 원본 이미지와 마스크 합성
    overlay = original_image * (1 - alpha * mask_3d) + colored_mask * (alpha * mask_3d)
    return overlay
...
def render_set(model_path, source_path, name, iteration, views, gaussians, pipeline, background, args,model=None):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        for i in range(len(view.sentence)):
            ...
            # 원본 이미지에 마스크 오버레이 (include_feature가 True일 때만)
...
def render_sets(dataset : ModelParams, checkpoint_path: str, pipeline : PipelineParams, skip_train : bool, skip_test : bool, args):
    with torch.no_grad():
        ...
        if not skip_train:
             render_set(dataset.model_path, dataset.source_path, "render_train", args.iteration, scene.getTrainCameras(), gaussians, pipeline, background, args)

        if not skip_test:
             render_set(dataset.model_path, dataset.source_path, "render", args.iteration, scene.getTestCameras(), gaussians, pipeline, background, args)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", "-c", default=None, type=str, help='Checkpoint filename inside model path or absolute path to .pth')
    parser.add_argument("--skip_train", action="store_true")
    parser.add_argument("--skip_test", action="store_true")
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--include_feature", action="store_true")
    args = get_combined_args(parser)
    # Respect CLI/config arguments. If a checkpoint isn't provided, search the model path for the latest .pth
    model_dir = args.model_path
    # Determine checkpoint path
    checkpoint_path = None
    if args.checkpoint:
        # If absolute or exists as given, use it; otherwise look inside model_dir
        if os.path.isabs(args.checkpoint) and os.path.exists(args.checkpoint):
            checkpoint_path = args.checkpoint
        else:
            candidate = os.path.join(model_dir, args.checkpoint)
        ...
        # Find latest .pth in model_dir
        pths = glob.glob(os.path.join(model_dir, "*.pth"))
        if len(pths) == 0:
            raise FileNotFoundError(f"No .pth files found in model path: {model_dir}. Provide --checkpoint with a valid file.")
        # pick newest by modification time
        checkpoint_path = max(pths, key=os.path.getmtime)

    # If iteration not provided, try to infer from checkpoint filename
    if args.iteration < 0:
        nums = re.findall(r"\d+", os.path.basename(checkpoint_path))
        args.iteration = int(nums[-1]) if len(nums) > 0 else -1

    render_sets(model.extract(args), checkpoint_path, pipeline.extract(args), args.skip_train, args.skip_test, args)
